# %%
"""
Pick-and-place example, potential-based reward shaping
"""
import numpy as np
...
for rollout in range(ROLLOUTS):
    ...
    for step in range(int(np.ceil(ROLLOUT_MAX_LEN * np.random.rand()))):
        ...
        shaped_reward = SHAPING.shaped_reward_function(
            ...
            plan=PLAN,
            shaping_mode='potential_based',
            gamma=GAMMA,
            add_1_to_reward=False,
            width=0.1
        )
        # potential function is scaled by K=30
        REWARDS[
            COUNTER
        ] = reward + 30*shaped_reward
